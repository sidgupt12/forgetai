...
export default function BrainPopup({ isOpen, onClose, onSubmit }) {
  ...
  return (
    ...
    >
      <motion.div
        ref={dialogRef}
        className="relative z-50 grid w-full max-w-[90%] md:max-w-[80%] xl:max-w-[70%] max-h-[80vh] overflow-hidden bg-white dark:bg-zinc-900 p-6 shadow-xl rounded-2xl border border-gray-200 dark:border-gray-700 flex flex-col" // Added flex flex-col
        tabIndex="-1"
      ...
      >
            ...
            <h2 className="text-xl font-bold leading-none tracking-tight">
              <div className="flex flex-row gap-2 items-center">
                <svg
                  ...
                  xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-1 text-green-500" // Slightly adjusted margin
                >
                  <path
        ...
        <div className="flex-grow overflow-hidden flex flex-col md:flex-row md:gap-4"> 
          {/* Tabs Navigation (Left Column on md+) */}
          <div
            dir="ltr" data-orientation="horizontal"
            className="flex-shrink-0 md:w-1/4 w-full mb-4 md:mb-0" // Adjusted width and margin
          >
            <div
              role="tablist" aria-orientation="vertical" // Changed to vertical for md+
              className="flex flex-row md:flex-col gap-2 md:gap-3 p-3 rounded-xl text-muted-foreground border bg-gray-50 dark:bg-zinc-800 shadow-sm overflow-x-auto md:overflow-y-auto md:h-full" // Adjusted flex direction and gap
          ...
          <div className="flex-grow overflow-y-auto md:w-3/4 w-full"> {/* Adjusted width and added overflow */}
            {/* Status Message Area */}
            <AnimatePresence>
        ...
        {/* Footer Buttons */}
        <div className="flex-shrink-0 flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 border-t border-gray-200 dark:border-gray-700 pt-4 mt-4">
          <button
            className="inline-flex items-center justify-center whitespace-nowrap rounded-lg text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 border border-gray-300 dark:border-gray-600 bg-white dark:bg-zinc-800 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-zinc-700 h-10 px-6 py-2 shadow-sm mt-2 sm:mt-0" // Added margin top for small screens
            type="button" onClick={onClose} disabled={isSubmitting}
          >
            Cancel
          </button>
          <button
            className={`inline-flex items-center justify-center whitespace-nowrap rounded-lg text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 bg-green-600 text-white shadow hover:bg-green-700 h-10 px-6 py-2 relative ${isSubmitting ? "opacity-70 cursor-not-allowed" : ""}`} // Added cursor style
            onClick={handleSubmit}
            disabled={ // Ensure disabled logic correctly checks for content in the active tab
              isSubmitting ||
              (activeTab === "note" && !content.note.trim()) || // Added trim()
              (activeTab === "document" && !content.document) || // Checks if document (File) is null
              (activeTab === "integrations" && !content.integrations.trim()) // Added trim()
            }
          >
            {isSubmitting ? (
              <>
